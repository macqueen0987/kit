...
3. **캐시된 옛 번들** — kit 번들은 `stale-while-revalidate=86400`으로 서빙된다. 방금 kit에 추가한 클래스를 쓰는 HTML을 배포하면, 재방문자는 **새 HTML + 최대 24시간 묵은 번들**을 받아 그 클래스만 무효가 된다(스펙 §11.6). kit에 새 클래스를 추가하는 마이그레이션은 **kit을 먼저 배포하고 Cloudflare purge로 반영을 확인한 뒤** 서비스 HTML을 배포한다. 브라우저에서 확인할 때는 반드시 강제 새로고침(Ctrl+Shift+R) 후 측정한다 — `curl`은 브라우저 캐시를 공유하지 않아 둘이 서로 다른 답을 준다.
